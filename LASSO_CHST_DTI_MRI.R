######## R codes for CHST ~ Char + DTI + MRI using LASSO ######
...
# Output file
# write.csv(preProc_summary, 'chst_lasso_char_dti_mri_summary.csv', row.names = FALSE)
...
# write.csv(coef.gglasso, 'chst_gglasso_woint_char_dti_mri.csv', row.names = FALSE)

gglasso_pred = predict(gglasso, test_ds_gglasso)
caret::RMSE(gglasso_pred, test_ds_lasso_y) # 3.309073