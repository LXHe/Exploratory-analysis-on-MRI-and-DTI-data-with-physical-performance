######## R codes for SPPB ~ Char + DTI + MRI using Random Forest ######
...
for (tree in seq(200, 2000, 200)){
  # Train RF model
  rf_cv = train(
    sppb_1 ~., data = train_ds_pf, method = "rf", tuneGrid = tunegrid, trControl = train_control, ntree = tree
  )
...
rf_opt = randomForest(
  sppb_1 ~., data = train_ds_pf, ntree = rf_best_ntree, 
...
# write.csv(rf_imp, 'sppb_rf_imp_char_dti_mri.csv', row.names = FALSE)
